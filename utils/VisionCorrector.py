...
class VisionCorrector:
    def __init__(self, target_size=(1000, 1000)):
        self.p = P() 
        ...
        anchor_center_offset = 3.5 * bs # 35.0
        ...
        self.dst_points = np.float32([
            ...
            [anchor_center_offset, h - anchor_center_offset] # BL
        ])
    ...
    def _find_anchor_centers(self, img):
        ...
        candidates = []
        # 在 _find_anchor_centers 找到 candidates 后增加
        if len(candidates) == 4:
            pts = np.array(candidates, dtype="float32").reshape(-1, 1, 2)
            # 亚像素精细化搜索（可选，如果环境光线好，建议开启）
        ...
        for i, cnt in enumerate(contours):
            ...
            if not (ideal_area * 0.3 < area < ideal_area * 1.5):
                continue
