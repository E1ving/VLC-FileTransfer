...
class VideoMuxer:
    ...
    @staticmethod
    def frames_to_video(frame_dir, output_path, fps=60):
        """使用 OpenCV 合成视频 (已优化)"""
    ...
    @staticmethod
    def ffmpeg_convert(frame_dir, output_path, fps=60):
        """
        ...
        try:
            ...
            print(f"✅ FFmpeg 编码完成: {output_path}")
            return True
        except (subprocess.CalledProcessError, FileNotFoundError):
            print("⚠️  FFmpeg 不可用，回退到 OpenCV 编码...")
            # 回退到 OpenCV 编码
            VideoMuxer.frames_to_video(frame_dir, output_path, fps)
            return False