...
class EncoderEngine:
    ...
    def create_base_frame(self):
        ...
        """
        img = np.ones((p.SCREEN_H, p.SCREEN_W), dtype=np.uint8) * 255
        res = p.ANCHOR_RESERVE  # 8

        for r in range(p.ROWS):
            for c in range(p.COLS):
                if p.is_in_anchor_zone(r, c):
                    # 获取局部坐标 (0-7)
                    lr = r if r < res else (r - (p.ROWS - res))
                    lc = c if c < res else (c - (p.COLS - res))
                    
                    is_right = c >= (p.COLS - res)
                    is_bottom = r >= (p.ROWS - res)
    ...
    def generate_all_frames(self, all_bits, output_dir="data/frames"):
        ...
        # 获取可用格子数（非锚点区）
        available_cells = p.get_available_cells()  # 6800
        frame_count = 0
        ...
        for i in range(0, len(all_bits), capacity):
            ...
            # 5. 确保 full_frame_bits 长度不超过可用格子数
            if len(full_frame_bits) > available_cells:
                print(f"警告：第 {frame_count} 帧的数据长度 ({len(full_frame_bits)}) 超过可用格子数 ({available_cells})，将截断")
                full_frame_bits = full_frame_bits[:available_cells]
