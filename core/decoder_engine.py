...
class DecoderEngine:
    def __init__(self):
        self.last_seq = -1 
        self.sample_radius = 1 # 针对 10px 格子，1 是理想半径
    ...
    def _get_robust_bit(self, img, x_c, y_c):
        ...
        return 1 if median_val > p.THRESHOLD else 0
    ...
    def process_frame(self, img):
        ...
        cursor = 0
        # 同步码 (16位)
        received_sync = all_bits[cursor : cursor + p.SYNC_BITS]
        cursor += p.SYNC_BITS
        ...
        # 帧序号 (8位)
        received_seq = self._bits_to_int(all_bits[cursor : cursor + p.SEQ_BITS])
        cursor += p.SEQ_BITS
        
        # 有效载荷长度 (16位)
        received_len = self._bits_to_int(all_bits[cursor : cursor + p.LEN_BITS])
        cursor += p.LEN_BITS
        ...
        # CRC 始终位于比特流的最后 16 位
        received_crc_bits = all_bits[-p.CRC_BITS:]
